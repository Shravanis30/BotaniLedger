...
import { Factory, PackageSearch, RefreshCw, Smartphone, List, CheckCircle2, QrCode, ArrowRight, PlusCircle, Loader2 } from 'lucide-react';
...
const ManufacturerDashboard = () => {
    ...
    const stats = [
        { l: "Available Verified", v: inventory.filter(b => b.blockchainRecord?.status === 'LAB_PASSED').length.toString().padStart(2, '0'), c: "text-primary", i: PackageSearch },
        ...
        <div className="flex bg-background min-h-screen">
            ...
            <main className="flex-1 ml-64 p-8">
                <header className="mb-10 flex justify-between items-end">
                    ...
                    <Link to="/manufacturer/products" className="px-6 py-3 bg-primary text-white rounded-2xl font-bold flex items-center gap-2 hover:bg-primary-mid transition-all shadow-lg shadow-green-900/10 active:scale-95">
                                            ...
                                            <tr key={batch._id || i} className="group hover:bg-primary/[0.02] transition-colors">
                                                ...
                                                <td className="px-8 py-6 text-right">
                                                    <Link to="/manufacturer/verify" className="text-xs font-bold text-primary hover:underline flex items-center justify-end gap-1">
                                                        Process Material <ArrowRight size={14} />
                                                    </Link>
                                                </td>
                                            </tr>
                                        ))
