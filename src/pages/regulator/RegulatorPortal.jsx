            ...
            <main className="flex-1 ml-64 p-10">
                <header className="mb-12 flex justify-between items-start">
                    <div className="flex gap-4 items-center">
                        <div className="w-16 h-16 bg-primary rounded-xl flex items-center justify-center text-white shadow-sm">
                            <ShieldCheck size={32} />
                        </div>
                        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
                    {complianceCards.map((s, i) => (
                        <Card key={i} className="p-8 border border-gray-200 shadow-sm relative overflow-hidden rounded-xl">
                            {statsLoading && (
                                <div className="absolute inset-0 bg-white/50 flex items-center justify-center z-20">
                                    <Loader2 className="animate-spin text-primary" size={20} />
                                </div>
                            )}
                            <div className="absolute top-0 right-0 w-24 h-24 bg-gray-50 rounded-bl-3xl -mr-12 -mt-12"></div>
                ...
                <section className="space-y-8">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="relative">
                            ...
                            <input className="pl-10 pr-6 py-3 bg-white border border-gray-200 rounded-xl outline-none focus:ring-2 ring-primary/20 w-64 text-sm font-medium" placeholder="Search Audit Ref..." />
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {logsLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-8 py-12 text-center text-gray-400">No audit records found</td>
                                    </tr>
                                ) : auditLogs?.map((log, i) => (
                                    <tr key={i} className="hover:bg-primary/5 transition-colors cursor-pointer">
                                        ...
                                        <td className="px-8 py-6">
                                            <span className="text-xs font-bold text-primary uppercase tracking-wider">{log.action}</span>
                                        </td>
                                        <td className="px-8 py-6">
